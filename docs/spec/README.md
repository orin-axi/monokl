...
Every verbatim code block here is meant to be exactly what gets written — this isn't illustrative pseudocode. If you're implementing a module and something looks inconsistent between files, check 05 first: it's the changelog for corrections made after the fact. Check 07 before trusting a failure path is handled — several commands' `diagnostics` fields and error branches look complete but aren't wired up.

## Known gaps

- `benches/pipeline.rs`'s four criterion groups are still stubs — none of the latency targets asserted anywhere in this spec are measured yet, including in monokl's own suite.
